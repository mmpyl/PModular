...
export default function BusinessSettingsPage() {
  const { token, organizationId, refreshMemberships } = useAuth(); const [organization, setOrganization] = useState<Organization | null>(null); const [name, setName] = useState(''); const [modules, setModules] = useState<string[]>([]); const [currency, setCurrency] = useState('PEN'); const [timezone, setTimezone] = useState('America/Lima'); const [defaultTaxRate, setDefaultTaxRate] = useState(0.18); const [message, setMessage] = useState(''); const [error, setError] = useState('');
